...
public class Facultad implements Informacion{
    ...
    public void elimiarEstudiante(Estudiante estudiante){
        Iterator<Estudiante> iterator = estudianteSet.iterator();

        while (iterator.hasNext()){
            String buscado = iterator.next().getNombre();
            if (buscado.equals(estudiante.getNombre())){
                System.out.println(estudiante.toString()+" fue eliminado");
                iterator.remove();
            }
        }

        Iterator<Carreras> iterator1 = carrerasSet.iterator();

        while (iterator1.hasNext()){
            String buscar = iterator1.next().getNombre();
            if (buscar.equals(estudiante.getNombre())){
                System.out.println(buscar+" fue eliminado");
                iterator1.remove();
            }
        }

        Iterator<Materias> materiasIterator = materiasSet.iterator();

        while (materiasIterator.hasNext()){
            String buscar = materiasIterator.next().getNombre();
            if (buscar.equals(estudiante.getNombre())){
                System.out.println(buscar+" fue eliminado");
